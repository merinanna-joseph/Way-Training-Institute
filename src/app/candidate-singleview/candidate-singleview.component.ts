...
export class CandidateSingleviewComponent implements OnInit {
  ...
  coaching_fee_div_value:number = 0;
  coaching_fee_paid:number;
  ...
  coaching_fee_paid_date;
  exam_fee_paid_date;
  convocation_fee_paid_date;
  attestation_fee_paid_date;
  equalency_fee_paid_date;
  other_fee_paid_date;
    ...
    total_paid_fees_peryear:number = 0;
    total_balance_fees_per_year:number = 0;
    registration_form_bind = 0;
    coaching_form_bind;
    exam_form_bind;
    convocation_form_bind;
    attestation_form_bind;
    equalency_form_bind;
    others_form_bind;
    previous_year_arrear_form_bind;
    previous_year_arrear_form_date_bind;
    previous_year_arrear_total;
    admiss_reg;
    admiss_coach;
    admiss_exam;
    admiss_conv;
      ...
      //fee flow calculation
        this.feeflowService.getFeeflowsWithStudentId(this.student_id).subscribe(
          feeflows => {
            ...
            this.total_fees_per_year = this.feeFlow.fee_per_year[0].total_fee + this.feeFlow.fee_per_year[0].previous_year_arrear;

            this.total_balance_fees_per_year = this.feeFlow.fee_per_year[0].balance_fee + this.feeFlow.fee_per_year[0].previous_year_arrear_balance;
 ...
 get f() { return this.addCollectFeeForm.controls; }
  ...
  getCourseTotal(){
    ...
    this.total_course_fees = Number(this.f.registrationAmount.value) +
  ...
  getRemittedTotal(){
    ...
    Number(this.f.miscellaneousAmount.value) + Number(this.f.transportAmount.value) + Number(this.f.previous_year_arrearAmount.value);
  }
 onYearChange(event){
 ...
 onFeeCollectSave(){
  ...
  for(var i=0;i<  this.allYears.length;i++){
    if(this.allYears[i] == this.f.fee_year.value){
      for(var c=0; c <  i; c++){
        if(this.feeFlow.fee_per_year[c].balance_fee > 0){
...
get cA() { return this.closeAdmissionForm.controls; }
  ...
  getBalanceTotal(){
    ...
    this.studentTotalDiscount = this.total_balance_fees_per_year - this.studentTotalPaidBalance;
  }
  onViewClick(student) {
