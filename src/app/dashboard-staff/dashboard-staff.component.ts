...
export class DashboardStaffComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.callEventService.getCalleventsByUser(this.auth.currentUser._id).subscribe(
      callEvents => {
        callEvents.reverse();
        callEvents.sort((a, b) => {
    ...
    this.leadService.getLeadsByAssgnedId(this.auth.currentUser._id).subscribe(
      leads =>{
        if(leads.length > 0){
          leads.reverse();
          for(var j in leads){
  ...
  onViewClick(callEvent:Callevent){
    this.route.navigate(['staff-single-lead/'+callEvent.leadId._id])
  }
